    ...
    AppCard(
        ...
        Column(
            ...
            Row(
                ...
                val noDataBPM = "-- bpm"
                HeartRateMetric("Min", if (minRate > 0) "$minRate bpm" else noDataBPM)
                HeartRateMetric("Avg", if (avgRate > 0) "$avgRate bpm" else noDataBPM)
                HeartRateMetric("Max", if (maxRate > 0) "$maxRate bpm" else noDataBPM)
            }
        }
    }
}

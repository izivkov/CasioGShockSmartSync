...
class IntentParser @Inject constructor() {
    ...
    // Supports "Set timer at four ten" -> 4 minutes 10 seconds
    ...
    fun parse(text: String): VoiceCommand? {
        ...
        alarmPatterns.firstNotNullOfOrNull { it.find(cleanedText) }?.let { match ->
            ...
            // 1. Try to parse as absolute time (e.g., "7:30 am")
            parseTime(timeString)?.let { time ->
                return VoiceCommand.SetAlarm(time.hour, time.minute)
            }
            
            // 2. Fallback: try to parse as relative duration (e.g., "3 hours from now")
            ...
            Timber.w("Failed to parse time string as absolute or relative: '$timeString'")
        }

        if (timerKeywordPattern.containsMatchIn(cleanedText)) {
            // 1. Try explicit units first: "3 minutes 10 seconds"
            ...
            // 2. Fallback to implicit units: "at four ten"
        ...
        reminderPatterns.firstNotNullOfOrNull { it.find(cleanedText) }?.let { match ->
            ...
            // Try to extract date/day from payload
        ...
        // Existing Auto Light / Power Saving logic
    ...
    private fun extractTitleAndDate(payload: String): Pair<String, LocalDate?> {
        ...
        // Fallback: search for date keywords in the whole payload
        parseDate(payload)?.let {
            // If date matched, we need to extract the title. This is hard.
            // Let's assume for now that if parseDate matched the whole payload, there's no title.
            // Or if it matched part of it...
        }
    ...
    private fun extractRepeat(payload: String): RepeatPeriod? {
        ...
        return when {
            ...
            lower.contains("no repeat") || lower.contains("don't repeat") -> RepeatPeriod.NEVER
            else -> null
        }
    }
    ...
    fun parseDate(text: String): LocalDate? {
        ...
        for ((name, day) in daysOfWeek) {
            if (lower.contains(name)) {
                ...
                if (target == today && lower.contains("next")) {
                    ...
                    // if today is Monday and we say "next Monday", it should be 7 days from now.
                    // nextOrSame(MONDAY) would give today.
        ...
        // Try parsing "October 5th", etc.
        // Simplified: just look for month name and a number
    ...
    private fun parseDurationToSeconds(text: String): Int {
        // Handle common fuzzy phrases like "hour and a half" -> "90 minutes"

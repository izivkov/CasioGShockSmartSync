...
private class GShockManagerImpl(
    ...
    override fun initialize() {
        Timber.i("initialize() called")
        super.initialize()
        ProgressEvents.onNext("BleManagerInitialized")
        Timber.i("initialize() complete")
    }

    // Called by Nordic's BleManager when it detects the device's services may
    // have changed (and on some reconnects) — clear any stale local state so
    // we don't hold onto characteristics from a previous, now-invalid GATT.
    override fun onServicesInvalidated() {
        Timber.i("onServicesInvalidated() called — clearing cached characteristics")

...
enum class READ_WRITE_MODE(value: Int) {
    READ(0xC),
    WRITE(0xE)
}
...
private class GShockManagerImpl(
    ...
    */
    ...
    override suspend fun write(handle: READ_WRITE_MODE, data: ByteArray) {
        ...
        val characteristic = if (handle == READ_WRITE_MODE.READ) readCharacteristicVar else writeCharacteristicVar
        val writeType = if (handle == READ_WRITE_MODE.READ) BluetoothGattCharacteristic.WRITE_TYPE_NO_RESPONSE else BluetoothGattCharacteristic.WRITE_TYPE_DEFAULT

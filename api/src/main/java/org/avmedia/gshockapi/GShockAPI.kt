...
class GShockAPI(private val context: Context) : IGShockAPI {
    ...
    /**
     * Get Health Data from the watch.
     *
     * @return [HealthData] containing steps, calories, heart rate, etc.
     */
    override suspend fun getHealthData(): HealthData {
        return HealthDataIO.request()
    }

    /**
     * Send Health Data to the watch.
     *
     * @param data [HealthData] to send.
     */
    override fun sendHealthData(data: HealthData) {
        HealthDataIO.sendToWatch(data)
    }
}
